...
function speakText(text, lang = "ne-NP") {
    ...
    // Strips out parenthetical script text (e.g. Devanagari) to keep audio reading clean
...
function translateENtoNEP(){
    let text = document.getElementById("enInput").value.trim().toLowerCase();
    ...
    if(englishToNepali[text]){
        ...
        output.innerHTML = `
            ...
            <button class="speak-btn" onclick="speakText(\`${translated}\`, 'ne-NP')">
                🔊 Listen
            </button>
        `;
    } else {
...
function translateNEPtoEN(){
    let text = document.getElementById("nepInput").value.trim().toLowerCase();
    ...
    if(nepaliToEnglish[text]){
        ...
        output.innerHTML = `
            <strong>${translated}</strong><br><br>
            <button class="speak-btn" onclick="speakText(\`${translated}\`, 'en-US')">
                🔊 Listen
            </button>
        `;
    } else {
...
// ================= EMERGENCY QUICK PHRASES DIRECT OUTPUT =================
